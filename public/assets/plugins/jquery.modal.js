    ...
    $.fn.modal = function(options) {
            if(options == 'close'){
                ...
                if(this[0]){
                    ...
                    $overlay.css({background: 'black', width: '100%', height: '100%', position: 'fixed', zIndex: 99999998, top: 0, left: 0}).hide();
                    $('body').append($overlay);

                    
                  	$modal = $(this).removeAttr('hidden');
            		...
            		$modal.css(cssOptions).hide().fadeIn().addClass('modal-window').removeClass('modal-window-hidden');
                }
                return this;
            }
